...
import { exportIncomesModuleExcel } from '../../lib/excelExportEngine';
import { FileSpreadsheet, Loader2 } from 'lucide-react';

export const IncomeView: React.FC = () => {
  const { incomes, categories, settings, addIncome, updateIncome, deleteIncome, bulkDeleteIncomes, filterRange } = useFinance();
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === paginatedIncomes.length && paginatedIncomes.length > 0) {
      setSelectedIds([]);
    } else {
  ...
  const handleExecuteBulkDelete = () => {
    if (selectedIds.length > 0) {
      bulkDeleteIncomes(selectedIds);
      setSelectedIds([]);
      setIsBulkDeleteConfirmOpen(false);
      setSuccessToast(`${selectedIds.length} ta daromad muvaffaqiyatli o'chirildi!`);
  ...
  const handleOpenModal = (income?: Income) => {
    if (income) {
      ...
      setFormDate(income.date.split('T')[0]);
      setFormCategoryId(income.category_id);
      setFormAmount(income.amount.toString());
      setFormDescription(income.description || '');
      setFormPaymentMethod(income.payment_method || 'Plastik karta');
    } else {
  ...
  const filteredIncomes = useMemo(() => {
    const list = incomes.filter((item) => {
  ...
  }, [incomes, filterRange, categoryFilter, minAmount, maxAmount, search, categories, sortField, sortOrder]);
  ...
  const handleExportExcel = async () => {
    try {
      ...
      await exportIncomesModuleExcel(filteredIncomes, categories, filterTitle);
      setSuccessToast('Kirimlar Excel fayli muvaffaqiyatli yuklab olindi!');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-4">
          <div className="p-3.5 bg-emerald-100/80 text-emerald-700 rounded-2xl">
            <TrendingUp className="w-7 h-7" />
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
              Tanlangan Davr Daromadi
            </span>
            <div className="text-2xl font-black text-slate-900 tracking-tight">
              {formatCurrency(totalFilteredAmount, settings.currency)}
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              Jami {filteredIncomes.length} ta operatsiya
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            title="Aynan ko'rib turgan filterlangan daromadlarni Excel formatida yuklab olish"
          >
            {isExporting ? (
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-semibold rounded-xl shadow-md shadow-emerald-600/20 transition-all"
          >
            <Plus className="w-4 h-4" />
            <span>Yangi Daromad</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            <select
              ...
              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-hidden focus:ring-2 focus:ring-emerald-500"
            >
              <option value="">Barcha daromad kategoriyalari</option>
        ...
        {(categoryFilter || minAmount || maxAmount || search) && (
          <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs text-slate-500">
            <span>Filtr qo'llanilgan ({filteredIncomes.length} ta natija)</span>
            <button
              onClick={() => {
                setCategoryFilter('');
                setMinAmount('');
          ...
          {selectedIds.length > 0 && (
            <div className="p-3 bg-slate-900 text-white rounded-2xl flex items-center justify-between shadow-lg animate-in slide-in-from-top-2">
              <div className="flex items-center gap-2 text-xs font-bold">
                <span className="px-2 py-0.5 bg-[#107c41] rounded-md">{selectedIds.length} ta tanlandi</span>
          ...
          {paginatedIncomes.length === 0 ? (
            ...
            paginatedIncomes.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`bg-white rounded-2xl border p-4 shadow-xs hover:border-emerald-200 hover:shadow-md transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 group ${
                    isSelected ? 'border-emerald-500 bg-emerald-50/20 ring-1 ring-emerald-500' : 'border-slate-200/80'
                  }`}
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    >
                      <CategoryIcon name={cat?.icon || 'TrendingUp'} className="w-5 h-5" />
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-slate-900 text-sm">
                          {cat?.name || 'Boshqa daromad'}
                        </span>
                      ...
                      <p className="text-xs text-slate-600 mt-0.5">
                        {item.description ? item.description : "Izohsiz daromad"}
                      </p>
        ...
        <div className="bg-white rounded-xl border border-[#cbd5e1] shadow-xs overflow-hidden font-sans">
          ...
          <div className="bg-[#f8fafc] px-4 py-2 border-b border-[#cbd5e1] flex items-center justify-between text-xs text-slate-600 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-slate-700 font-semibold">=SUM(G2:G{paginatedIncomes.length + 1})</span>
              {selectedIds.length > 0 && (
            ...
            <div className="text-[11px] font-mono text-slate-500">
              Jami tanlangan daromad: <strong className="text-[#107c41] font-bold">{formatCurrency(totalFilteredAmount, settings.currency)}</strong>
            </div>
          </div>
                ...
                <tr className="border-b border-[#cbd5e1]">
                  <th className="w-10 px-2 py-2.5 text-center bg-[#e2e8f0] border-r border-[#cbd5e1]">
                    <button onClick={handleSelectAll} className="flex items-center justify-center mx-auto text-slate-600">
                      {selectedIds.length === paginatedIncomes.length && paginatedIncomes.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center justify-between gap-1">
                      <span>Sana</span>
                      {sortField === 'date' ? (
                        sortOrder === 'asc' ? <ArrowUp className="w-3 h-3 text-[#107c41]" /> : <ArrowDown className="w-3 h-3 text-[#107c41]" />
                      ) : (
              ...
              <tbody className="divide-y divide-slate-200 text-slate-700 font-sans">
                {paginatedIncomes.length === 0 ? (
                  ...
                  paginatedIncomes.map((item, index) => {
                    ...
                    const isSelected = selectedIds.includes(item.id);
                    return (
                      <tr key={item.id} className={`transition-colors ${isSelected ? 'bg-emerald-50/70' : 'hover:bg-emerald-50/40'}`}>
                        ...
                        <td className="px-3.5 py-2.5 whitespace-nowrap border-r border-[#e2e8f0]">
                          <div className="flex items-center gap-1.5">
                            <span
                              ...
                              style={{ backgroundColor: cat?.color || '#107c41' }}
                            />
                        ...
                        <td className="px-3.5 py-2.5 text-right font-bold text-[#107c41] whitespace-nowrap font-mono border-r border-[#e2e8f0]">
                          +{formatCurrency(item.amount, settings.currency)}
                        </td>
                        <td className="px-3.5 py-2.5 text-center whitespace-nowrap">
                          <div className="flex items-center justify-center gap-1">
                            <button
                              onClick={() => handleOpenModal(item)}
                              className="p-1 text-slate-500 hover:text-[#107c41] hover:bg-emerald-50 rounded transition-colors"
                              title="Tahrirlash"
                            >
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
              ...
              {paginatedIncomes.length > 0 && (
                <tfoot className="bg-[#f1f5f9] border-t-2 border-[#107c41] text-xs font-bold text-slate-900">
                  <tr>
                    ...
                    <td className="px-3.5 py-2.5 text-right text-[#107c41] font-mono text-sm border-r border-[#cbd5e1] border-b-4 border-double border-[#107c41]">
                      +{formatCurrency(totalFilteredAmount, settings.currency)}
                    </td>
                    <td></td>
                  </tr>
            ...
            <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-base font-bold text-slate-900">
                  {editingIncome ? "Daromadni Tahrirlash" : "Yangi Daromad Kiritish"}
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-base font-bold text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Masalan: Avgust oyi avansi"
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-hidden focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl shadow-sm shadow-emerald-600/20"
                >
                  <Check className="w-4 h-4" />
                  <span>{editingIncome ? "O'zgarishlarni Saqlash" : "Daromadni Saqlash"}</span>
                </button>
              </div>
            </form>
          </div>
      ...
      <ConfirmDialog
        ...
        message="Haqiqatan ham ushbu daromad yozuvini o'chirmoqchimisiz? Ushbu amaldan so'ng balans qayta hisoblanadi."
      ...
      <ConfirmDialog
        ...
        title="Ommaviy Daromadlarni O'chirish"
        message={`Haqiqatan ham tanlangan ${selectedIds.length} ta daromad yozuvini o'chirmoqchimisiz? O'chirilgan ma'lumotlar Chiqindilar qutisiga (Recycle Bin) o'tadi va ularni keyinchalik qayta tiklash mumkin.`}
